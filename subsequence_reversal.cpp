...
void init()
{
	setIO("subrev");// WHEN YOU CODE WITH LL BE SURE TO USE LINF AS INITIALIZER INSTEAD OF INF.
	cin >> n;
	A.rsz(n);
	F0R(i, n)cin >> A[i];
	trav(x, A)x--;
...
int main()
{
	...
	dp.rsz(n, vector<vector<vi>>(n, vector<vi>(MAXN, vi(MAXN))));
	F0R(i, MAXN)FOR(j, i, MAXN)F0R(k, n)
		dp[k][k][i][j] = (A[k] >= i && A[k] <= j);
	F0R(i, MAXN)F0R(j, i - 1)F0R(k, n)F0R(l, n)
		dp[k][l][i][j] = -inf;
	FOR(d, 1, n)
	{
		F0R(i, n)
		{
			...
			F0R(k, MAXN)
			{
				FOR(l, k, MAXN)
				{
					...
					if (A[i] >= k)
						dp[i][j][k][l] = max(dp[i][j][k][l], dp[i + 1][j][A[i]][l] + 1);
					if (A[j] <= l)
						dp[i][j][k][l] = max(dp[i][j][k][l], dp[i][j - 1][k][A[j]] + 1);
					if (A[j] >= k)
						dp[i][j][k][l] = max(dp[i][j][k][l], dp[i + 1][j - 1][A[j]][l] + 1);
					if (A[i] <= l)
						dp[i][j][k][l] = max(dp[i][j][k][l], dp[i + 1][j - 1][k][A[i]] + 1);
					if (A[i] <= l && A[j] >= k)
						dp[i][j][k][l] = max(dp[i][j][k][l], dp[i + 1][j - 1][A[j]][A[i]] + 2);
				}
			}
		}
	}
	cout << dp[0][n - 1][0][MAXN - 1] << endl;
}