...
void init()
{
	setIO("tallbarn");
	cin >> n >> k;
	A.rsz(n); F0R(i, n) { cin >> a; A[i] = a; }
}
ll solve(int i, ld val)
{
	return ceil(-0.5 + sqrt(A[i] / val + 0.25));
}
bool check(ld val)
{
	vll count(n);
	F0R(i, n)count[i] = solve(i, val);
	ll a = accumulate(all(count), (ll)0);
	return a <= k;
}
struct val
{
...
bool comparator(val x, val y)
{
	return (ld)A[x.index] / (x.count * (x.count + 1)) < (ld)A[y.index] / (y.count * (y.count + 1));
}
int main()
{
	init();
	...
	low = high;
	pq<val, vector<val>, decltype(&comparator)>vals(&comparator);
	F0R(i, n) { a = solve(i, low); k -= a; val temp = { i,a }; vals.push(temp); }
	F0R(i, k)
	{
		val top = vals.top();
		vals.pop();
	...
	while (vals.size())
	{
		val top = vals.top();
		vals.pop();
		answer += (ld)A[top.index] / top.count;
	}
	cout << (ll)round(answer) << endl;
}