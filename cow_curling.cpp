...
void init()
{
	...
	F0R(i, n)cin >> B[i].x >> B[i].y;
}
int solve(vector<point>& hull, const point& center, const point& p)
{
	bool flag = 0;
	ll d;
	F0R(i, hull.size() - 1)
	{
		c = area(hull[i], hull[i + 1], center);
		d = area(hull[i], hull[i + 1], p);
		if (d == 0)
		{
			if ((hull[i].x > p.x || hull[i + 1].x < p.x) || (hull[i].y > p.y || hull[i + 1].y < p.y))
				flag = 1;
			else
				return 1;
		}
	...
	c = area(hull[hull.size() - 1], hull[0], center);
	d = area(hull[hull.size() - 1], hull[0], p);
	if (d == 0)
	{
		if ((hull[n - 1].x > p.x || hull[0].x < p.x) || (hull[n - 1].y > p.y || hull[0].y < p.y))
			flag = 1;
		else
			return 1;
	}
...
point p_B;
point p;
bool __convex_sort(const point& x, const point& y)
{
