...
int main()
{
	...
	//print_vector(points);
	/*
	vector<vector<vector<bool>>>above;//is j above or below i->k
	above.rsz(n);
	F0R(i, n)above[i].rsz(n);
	F0R(i, n)F0R(j, n)above[i][j].rsz(n);
	F0R(i, n)
	{
		F0R(j, n)
		{
			F0R(k, n)
			{
				above[i][j][k] = 1 - clockwise(points[i], points[k], points[j]);
			}
		}
	}*/
	//print_vector(above);

	vector<vi>above_count;
	above_count.rsz(n);
	F0R(i, n)above_count[i].rsz(n);
	F0R(i, n)FOR(j, i + 1, n)
	{
		FOR(k, i + 1, j)
		{
			if (/*above[i][k][j]*/clockwise(points[i], points[j], points[k]) == 0)above_count[i][j]++;
		}
	}
	//print_vector(above_count);
	vi count;
	count.rsz(n - 2);
	F0R(i, n)
	{
		FOR(j, i + 1, n)
		{
			FOR(k, j + 1, n)
			{
				//if (i == 0 && j == 4 && k == 6)
				//	a++;
				if (/*above[i][j][k]*/clockwise(points[i], points[j], points[k]) == 1)
				{
					a = above_count[i][k] - above_count[i][j] - above_count[j][k] - 1;
				}
				else
				{
					a = above_count[i][k] - above_count[i][j] - above_count[j][k];
					a = -a;
				}
				//cout << i << " " << j << " " << k << ": \t" << a << endl;
				count[a]++;
