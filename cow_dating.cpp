...
int main()
{
	...
	F0R(i, n)
	{
		long double d = 1 - values[i];
		ratio[i] = (values[i] / d);
	}
	//trav(x, values)cout << x << endl;
	//trav(x, ratio)cout << x << endl;
	vector<long double>prefix_sum;
	prefix_sum.rsz(n + 1);
	F0R(i, n)prefix_sum[i + 1] = prefix_sum[i] + ratio[i];
	//trav(x, prefix_sum)cout << x << endl;
	//cout << endl;
	vector<long double>prefix_product;
	prefix_product.rsz(n + 1);
	prefix_product[0] = 1;
	F0R(i, n)prefix_product[i + 1] = prefix_product[i] * (1 - values[i]);
	//trav(x, prefix_product)cout << x << endl;
	long double max = 0;
	F0R(i, n)
	{
		int index = upper_bound(all(prefix_sum), 1 + prefix_sum[i]) - prefix_sum.begin();
		if (index == n + 1)index--;
		long double val = prefix_sum[index] - prefix_sum[i];
		val *= (prefix_product[index] / prefix_product[i]);
		if (max < val)
			max = val;
		//cout << max << ednl;
	}
	cout << (int)(max * 1e6) << endl;
}