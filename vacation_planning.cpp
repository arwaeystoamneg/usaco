...
vector<vpi>parents;
vi hub;
vector<vi>B;// point to hub
vector<vi>A;// hub to point
void init()
{
	setIO("vacationgold");
	cin >> n >> m >> k >> q;
	...
	A.rsz(k);
	F0R(i, k)A[i].rsz(n, inf);
	B.rsz(k);
	F0R(i, k)B[i].rsz(n, inf);
}
void dijkstra1(int i)
{//Preconditions for this to work: 1. no negative edges. 2. dist and parents are both initialized with size N and full of INT_MAX and 0 respectively while dist[i]=0 and parent[i]=-1
	priority_queue<pii>todo;
	vi finalized;
	finalized.rsz(n + 1, 0);//make sure that the size of adjacency is N+1
	todo.push(mp(0, hub[i]));
	while (!todo.empty())
	{//.s is indexs while .f is weight
		pii temp = todo.top();
		int indexs = temp.second;
		...
		trav(x, adj[indexs])
		{
			if (finalized[x.first])
				continue;
			if (A[i][x.f] > x.s + A[i][indexs])
			{
				A[i][x.first] = x.second + A[i][indexs];
				todo.push(mp(-A[i][x.first], x.f));
			}
		}
	}
}
void dijkstra2(int i)
{//Preconditions for this to work: 1. no negative edges. 2. dist and parents are both initialized with size N and full of INT_MAX and 0 respectively while dist[i]=0 and parent[i]=-1
	priority_queue<pii>todo;
	vi finalized;
	finalized.rsz(n + 1, 0);//make sure that the size of adjacency is N+1
	...
	{//.s is indexs while .f is weight
		...
		if (finalized[indexs])continue;
		finalized[indexs] = 1;
		trav(x, parents[indexs])
		{
			if (finalized[x.first])
				continue;
			if (B[i][x.f] > x.s + B[i][indexs])
			{
				B[i][x.first] = x.second + B[i][indexs];
				todo.push(mp(-B[i][x.first], x.f));
			}
		}
	}
}
