...
int main()
{
	...
	ROF(i, 1, n - 1)
	{
		...
		FOR(j, 1, m - 1)
		{
			if (types[i][j] && types[i - 1][j])
			{
				if (y.find(types[i][j]) != y.end())
				{
					if (!same(types[i - 1][j], y[types[i][j]]))
						unite(types[i - 1][j], y[types[i][j]]);
				}
				else
				{
					y.insert({ types[i][j],types[i - 1][j] });
				}
			}
		}
		F0R(j, m)if (types[i - 1][j])types[i - 1][j] = find(types[i - 1][j]);
	}
	//cout<<a<<endl;
	vll dp;
	dp.rsz(a, 1);
	set<int>t;
	F0R(i, n)F0R(j, m)if (types[i][j])t.insert(types[i][j]);
	vi r;
	trav(x, t)r.pb(x);
	map<int, int>index;
	F0R(i, sz(r))index.insert({ r[i],i });
	F0R(i, n)F0R(j, m)(types[i][j] ? types[i][j] = index[types[i][j]] : types[i][j] = a);//change to -1?
	//print_vector(types);
	...
	FOR(i, 1, n - 1)
	{
		FOR(j, 1, m - 1)
		{
			if (types[i][j] != a)
			{
				if (types[i + 1][j] != a)
				{
					adjacency[types[i][j]].insert(types[i + 1][j]);
					parents[types[i + 1][j]].insert(types[i][j]);
				}
			}
		}
	}
	...
	list<int>todo;
	F0R(i, a)if (!adjacency[i].size())todo.pb(i);
	vi sizes;
	sizes.rsz(a);
	F0R(i, a)sizes[i] = sz(adjacency[i]);
	vi order;
	while (!todo.empty())
	{
		int index = todo.back();
	...
	//print_vector(dp);
	set<int>o;
	F0R(i, n)F0R(j, m)o.insert(types[i][j]);
	if (o.size() == 1)
	{
		cout << 1 << endl;
		return 0;
