...
int main()
{
	setIO("cbs");
	cin >> k >> n;
	a.rsz(k); trav(x, a)cin >> x;
	prefix.rsz(k, vi(n + 1));
	F0R(i, k)
	{
		F0R(j, n)prefix[i][j + 1] = prefix[i][j] + (a[i][j] == '(') - (a[i][j] == ')');
	}
	vector<vi>dp(k, vi(n + 1));
	F0R(i, k)
	{
		map<int, int>index_;
		R0F(j, n + 1)
		{
			index_[prefix[i][j]] = j;
			auto it = index_.find(prefix[i][j]);
			if (it == index_.begin())dp[i][j] = n;
			else
			{
				it--;
				dp[i][j] = it->s - 1;
