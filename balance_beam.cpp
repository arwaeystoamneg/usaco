...
int main()
{
	...
	//trav(x, points)cout << x.x << " " << x.y << endl;
	convex_hull();
	//trav(x, hull)cout << x.x << " " << x.y << ednl;
	vector<long double>answer;
	answer.rsz(n);
	FOR(i, 1, hull.size())
	{
		int dist = hull[i - 1].x - hull[i].x;
		R0F(k, dist + 1)
		{
			answer[hull[i].x + k] = (((long double)dist - k) / dist * (hull[i].y) + (long double)k / dist * hull[i - 1].y);
		}
	}
	//trav(x, answer)cout << x << ednl;
	FOR(i, 1, n - 1)
	{
		cout << (ll)floor((answer[i] + 1e-10) * 1e5) << ednl;
	}
}