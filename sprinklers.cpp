...
void build()
{
	vll changes(n), d(n);
	ll total = 0;
	trav(x, update)if (x) { changes[x - 1]++, total++; }
	update.rsz(0);
	F0R(i, n)
	{
		d[i] += total;
...
int main()
{
	...
	vll query(n);
	query[n - 1] = a[n - 1];
	R0F(i, n - 1)query[i] = max(query[i + 1], a[i]);
	ll ans = 0, total = 0;
	ll j = n;
	F0R(i, n)
	{
		...
		trav(x, update)
		{
			...
			{
				ans += query[i] * (query[i] + 1) / 2;
			}
			ans %= inf;
		}
		j = min(j, a[i]);
		update.pb(j);
		total += i * (query[i] + 1) * query[i] / 2;
		total %= inf;
	}
	cout << (total - ans + inf) % inf << endl;
}