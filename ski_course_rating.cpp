...
int main()
{
	...
	F0R(i, n)
	{
		F0R(j, m)
		{
			F0R(k, 2)
			{
				if (check(i + dx[k], j + dy[k]))e.pb({ m * i + j,m * (i + dx[k]) + j + dy[k],abs(a[i][j] - a[i + dx[k]][j + dy[k]]) });
			}
		}
	}
	sort(all(e));
	//trav(x, e)cout << x.a << " " << x.b << " " << x.len << endl;
	link.rsz(n * m); iota(all(link), 0);
	sizes.rsz(n * m, 1);
	todo.rsz(n * m);
	F0R(i, n)
	{
	...
	trav(t, e)
	{
		...
		if (x != y)
		{
			...
			link[x] = y;
			sizes[y] += sizes[x];
			todo[y].splice(todo[y].end(), todo[x]);
			if (sizes[y] >= T)
			{
				trav(x, todo[y])ans += t.len;
				todo[y].clear();
			}
		}
	}
