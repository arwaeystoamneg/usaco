...
{
	if ((acc % 2) != (A[index] % 2))return 0;
	pq<int>temp;
	F0R(i, index)if (A[i])temp.push(A[i]);
	FOR(i, index + 1, n + 1)if (A[i])temp.push(A[i]);
	while (!temp.empty())
	{
		int t = temp.top();
		temp.pop();
...
int main()
{
	...
	//cout << endl;
	F0R(i, n + 1)A[i] = answer[A[i]];
	cout << accumulate(all(A), 0) << endl;
	F0R(i, n + 1)if (A[i])cout << i + 1 << endl;
}