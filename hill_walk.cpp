...
long double area(pll x, pll y, pll z)
{
	return (x.s * y.f + y.s * z.f + z.s * x.f - x.f * y.s - y.f * z.s - z.f * x.s) / 2.0;
}
struct edge
{
	int a, b, c, d;
	...
	{
		ld area1 = area(mp(a, b), mp(c, d), mp(x.a, x.b)), area2 = area(mp(a, b), mp(c, d), mp(x.c, x.d));
		if (area2 == 0)swap(area1, area2);
		if (area1 < 0)
		{
			if (area2 < 0)return 1;
			else
...
map<edge, int, co>index_;
int main()
{
	setIO("hillwalk");
	cin >> n;
	a.rsz(n);
	trav(x, a)cin >> x.a >> x.b >> x.c >> x.d;
	F0R(i, n)todo.pb({ a[i].a,i,0 }), todo.pb({ a[i].c,i,1 });// 0 is add, 1 is remove
	sort(all(todo));
	F0R(i, n)index_.insert({ a[i],i });
	int i = 0;
	int ans = 1;
	trav(x, todo)
	{
