...
int main()
{
	...
	{
		cin >> a;
		if (a < 0)A.pb(a); else B.pb(a);
	}
	int a = A.size(), b = B.size();
	A.pb(0); B.pb(0);
	sort(all(A), greater<int>()); sort(all(B));
	vector<vector<vll>>dp(a + 1, vector<vll>(b + 1, vll(2)));
	...
	R0F(i, a + 1)
	{
		R0F(j, b + 1)
		{
			...
			if (i == a)
			{
				dp[i][j][0] = dp[i][j + 1][1] + (B[j + 1] - A[i]) * (n - i - j);
				dp[i][j][1] = dp[i][j + 1][1] + (B[j + 1] - B[j]) * (n - i - j);
			}
			else if (j == b)
			{
				dp[i][j][0] = dp[i + 1][j][0] + (abs(A[i + 1] - A[i])) * (n - i - j);
				dp[i][j][1] = dp[i + 1][j][0] + (abs(A[i + 1] - B[j])) * (n - i - j);
			}
			else
			{
				dp[i][j][0] = min(dp[i + 1][j][0] + (abs(A[i + 1] - A[i])) * (n - i - j), dp[i][j + 1][1] + (abs(B[j + 1] - A[i])) * (n - i - j));
				dp[i][j][1] = min(dp[i + 1][j][0] + (abs(A[i + 1] - B[j])) * (n - i - j), dp[i][j + 1][1] + (abs(B[j + 1] - B[j])) * (n - i - j));
			}
		}
	}
	cout << dp[0][0][0] << endl;
