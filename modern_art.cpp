...
int n;
vector<vi>A;
vector<pair<point, point>>range;
vector<vector<change>>add, sub;
void init()
{
	setIO("art");
	cin >> n;
	...
	t.x = -1, t.y = -1;
	range.rsz(n * n + 1);
	F0R(i, n * n + 1)range[i] = { t,t };
	F0R(i, n)F0R(j, n)
	{
		if (A[i][j])
		{
			if (range[A[i][j]].f.x == -1)range[A[i][j]].f.x = i;
			if (range[A[i][j]].f.y == -1)range[A[i][j]].f.y = j;
			range[A[i][j]].f.x = minimum(range[A[i][j]].f.x, i);
			range[A[i][j]].f.y = minimum(range[A[i][j]].f.y, j);
			range[A[i][j]].s.x = maximum(range[A[i][j]].s.x, i);
			range[A[i][j]].s.y = maximum(range[A[i][j]].s.y, j);
		}
	}
	//cout << endl;
	//trav(x, range)cout << x.f.x << " " << x.f.y << " " << x.s.x << " " << x.s.y << endl;
	add.rsz(n + 1);
	sub.rsz(n + 1);
	a = 0;
	F0R(i, n * n + 1)
	{
		if (range[i].f.x != -1)
		{
			...
			r.l = range[i].f.x;
			r.r = range[i].s.x;
			add[range[i].f.y].pb(r);
			sub[range[i].s.y + 1].pb(r);
			a++;
		}
	}
	/*
	...
	{
		cout << i << endl;
		vector<change>v = add[i];
		trav(x, v)cout << x.l << " " << x.r << " " << x.type << endl;
	}
	F0R(i, sub.size())
	{
		cout << i << endl;
		vector<change>v = sub[i];
		trav(x, v)cout << x.l << " " << x.r << " " << x.type << endl;
	}
	*/
}
int main()
{
	...
	F0R(i, n)F0R(j, n + 1)
	{
		...
		if (colors.size() > 1)
		{
			//trav(x, colors)if (elim.find({ A[i][j],x }) != elim.end()) { cout << -1 << endl; return 0; }
			//trav(x, colors)if (x != A[i][j])elim.insert({ x,A[i][j] });
			possible[A[i][j]] = 0;
		}
		if (j < n && A[i][j] == 0 && colors.size())
		{
			cout << -1 << endl;
			return 0;
		}
	...
	if (a > 1)
		cout << accumulate(all(possible), 0) << endl;
	else
		cout << accumulate(all(possible), -1) << ednl;
}