...
public:void build(int n)
{
	...
	tree.rsz((1 << height + 1) - 1, neutral);
	fill(all(A), neutral);
	fill(all(tree), neutral);
}
...
int main()
{
	...
	segment_tree above, below;
	above.build(n + q); below.build(n + q);
	F0R(i, n + q)
	{
		if (A[i].type == 0)
		{
			above.update(indexs[A[i].y], -A[i].x + A[i].y + A[i].t);
			below.update(indexs[A[i].y], -A[i].x - A[i].y + A[i].t);
		}
		else
		{
			answer[A[i].index] = minimum(answer[A[i].index], A[i].x - A[i].y + above.query(indexs[A[i].y], n + q));
			answer[A[i].index] = minimum(answer[A[i].index], A[i].x + A[i].y + below.query(0, indexs[A[i].y]));
		}
	}
	above.build(n + q); below.build(n + q);
	R0F(i, n + q)
	...
	trav(x, A)if (x.type == 1)answer[x.index] = minimum(answer[x.index], abs(x.x - x.y));
	F0R(i, q)cout << answer[i] << endl;
}