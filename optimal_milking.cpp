...
class segment_tree
{
	...
	{
		item ans;
		ans.both = max({ x.both + y.second, x.first + y.second, x.first + y.both });
		ans.first = max({ x.both + y.neither, x.first + y.first, x.first + y.neither });
		ans.second = max({ x.neither + y.both, x.neither + y.second, x.second + y.second });
		ans.neither = max(x.second + y.neither, x.neither + y.first);
		return ans;
	}
	vector<item> tree;
	vector<item>A;
...
public:ll query(int l, int r)
{
	...
	return max({ ans.both, ans.second, ans.first, ans.neither });
}
