...
{
	return query(0, 0, (1 << height) - 1, l, r).ans;
}
...
int main()
{
	...
	R0F(i, n)
	{
		sum.update(2 * n - 1 - A[i], 1);
		sum.update(2 * n - 1 - B[i], 2);
		ans = max(ans, dp[i] + sum.query(0, 2 * n - 1));
	}
	cout << ans << endl;
}