...
void solve(int val)
{
	...
	F0R(i, n)
	{
		if (val & (1 << i))
		{
			if (dp[val ^ (1 << i)] != -1)
			{
				int index = upper_bound(all(times[i]), dp[val ^ (1 << i)]) - times[i].begin();
				if (index != 0)
				{
					index--;
					max = maximum(max, times[i][index] + duration[i]);
				}
			}
		}
	}
...
int main()
{
	...
	F0R(i, total)
	{
		...
		if (dp[i] >= k)min = minimum(min, count);
	}
	//print_vector(dp);
	if (min == n + 1)cout << -1 << endl;
	else cout << min << endl;
}